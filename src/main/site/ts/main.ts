...
const suggestionList = document.getElementById("suggestions") as HTMLSelectElement
...
function postAndUpdate(): void {
  ...
  fetch('http://localhost:4567/path', {
    // Request method
    method: 'post',
...
function updateSuggestions(matches: string[]): void {
  ...
  matches.forEach((match) => suggestionList.innerHTML += "<li tabindex=\"0\">" + match + "</li>")
}
...
document.addEventListener("keyup", function (event) {
  if (event.key === "a") {
    updateValues("Scorpio", "Scorpio", "Scorpio").then(postAndUpdate)
  }
})
